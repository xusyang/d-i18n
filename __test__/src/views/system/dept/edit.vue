<template>
  <el-dialog :title="title" v-model="show" width="600px" append-to-body>
    ...
    >
      <el-form-item :label="I18N.$fanyi('所属部门')" prop="deptId">
        <DCacaderDept
          class="full"
      ...
      <el-form-item :label="I18N.$fanyi('部门编码')" prop="deptCode">
        <el-input
          class="full"
          v-model="formData.deptCode"
          :placeholder="I18N.$fanyi('请输入部门编号')"
          maxlength="30"
        />
      </el-form-item>

      <el-form-item :label="I18N.$fanyi('部门类型')">
      ...
      <el-form-item :label="I18N.$fanyi('部门状态')">
...
const data = reactive({
  ...
  // 新增还是编辑
  formData: { ...rawFormData, id: null },
  formRules: {
    deptName: [
      {
        required: true,
        message: I18N.$fanyi('请输入部门名称'),
        trigger: 'blur'
      }
    ],
    deptCode: [
...
const title = computed(() => {
  if (type.value === MODEL_OPERATOR_TYPE.新增) {
    return I18N.$fanyi('新增部门')
  } else {
    return I18N.$fanyi('编辑部门')
  }
})
/**
 * 打开部门编辑
