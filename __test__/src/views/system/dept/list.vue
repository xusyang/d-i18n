    ...
    <template #query>
      <el-form label-position="top" :model="queryData" ref="refQueryForm" :inline="true">
        <el-form-item label="部门名称" prop="deptName">
          <el-input v-model="queryData.deptName" placeholder="请输入部门名称" clearable @keyup.enter="handleQuery" />
        </el-form-item>
        <el-form-item label="部门类型" prop="deptType">
          <el-select v-model="queryData.deptType" placeholder="请选择部门类型" clearable>
            <el-option v-for="item in deptOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="部门状态" prop="status">
          <el-select v-model="queryData.status" placeholder="部门状态" clearable>
...
import { watch, getCurrentInstance, onMounted, reactive, ref, toRefs, inject } from 'vue'
...
import { convertEnumToOptions, convertEnumValueToLabel, ENUM_DEPT_TYPE, ENUM_STATUE_TYPE } from '@/constants/enum'
...
const getData = async () => {
  ...
  // tableDatas.value = proxy.handleTree(res.data, 'deptId')
...
const handleDelete = async (row) => {
  deleteRef.value.open('确定要删除当前菜单？如果当前菜单包含子集，则子集的内容也将被一并删除，是否确认删除？', { deptId: row.id })
}
...
const tableColumns = reactive<IDTableColumn[]>([
  { prop: 'legalName', label: '所属法人', width: 200, align: 'center' },
  { prop: 'legalName', label: '所属部门', width: 200, align: 'center',
    formatter: (row) => {
      const findOne = tableDatas.value.find(item => item.id === row.parentId)
      return findOne ? findOne.deptName : '-'
    },
  },
  { prop: 'deptCode', label: '部门编码', width: 160, align: 'left' },
  ...
  { prop: 'orderNum', label: '排序序号', width: 80, align: 'center' },
  {
    prop: 'status',
    label: '状态',
    width: 80,
