<template>
  <DTable :tableDatas="tableDatas" :tableColumns="tableColumns" :setting="false" row-key="id" :height="850">
    <template #pagination>
      <Pagination :total="total" :page="pageParams.page" :limit="pageParams.limit" @pagination="handlePagenation" />
    </template>
  </DTable>
</template>
...
const getConditions = () => {
  try {
    ...
    return [
      { fieldCode: CPS_VIEW_QUERY_CODE[defineKey], op: 'eq', values: [tableCode] },
      { fieldCode: 'period', op: 'eq', values: [period] },
    ]
  }
}
  ...
  getViewDefineById(CPS_VIEW_ID[defineKey]).then((res) => {
    ...
    tableColumns.value = viewJson.showFields.map((item) => {
      return {
        ...
        formatter: (row) => {
          return item.template_type === 'raw' ? row[item.fieldCode] : fieldFormate(row, item.fieldCode)
        },
      }
    })
...
const fieldFormate = (row, prop) => {
  ...
  if (customArchives[prop] && allCustomData.value.length) {
    const ret = allCustomData.value.find((item) => item.item_code === originalVal)
    return ret ? ret.item_name : '-'
  } else if (objectField[prop] && allTreeData.value.length) {
    let code = prop.indexOf('cate') > -1 ? `${objectField[prop]}_cate_code` : `${objectField[prop]}_code`
    let name = prop.indexOf('cate') > -1 ? `${objectField[prop]}_cate_name` : `${objectField[prop]}_name`
