  ...
  <div>
    <el-dialog :title="title" v-model="show" width="400px" append-to-body>
      ...
      >
        <el-row>
          <el-col :span="24">
            <el-form-item :label="I18N.$fanyi('所属集团')" prop="groupId">
              <el-input
                v-if="formData.groupId == '0'"
                v-model="not"
                disabled
              ></el-input>
              <DSelectGroup
                v-else
                v-model:groupId="formData.groupId"
                v-model:groupName="formData.groupName"
                class="full"
              />
            </el-form-item>
          </el-col>
          ...
          <el-col :span="24">
            <el-form-item :label="I18N.$fanyi('法人名称')" prop="legalName">
              <el-input
                class="full"
                v-model="formData.legalName"
                :placeholder="I18N.$fanyi('请输入法人名称(不超过50个字符)')"
                maxlength="50"
              />
            </el-form-item>
          </el-col>
          ...
          <el-col :span="24">
            <el-form-item :label="I18N.$fanyi('法人编码')" prop="legalCode">
              <el-input
                class="full"
                v-model="formData.legalCode"
                :placeholder="I18N.$fanyi('请输入法人编码')"
              />
            </el-form-item>
          </el-col>
        </el-row>
...
const checkGroup = (rule: any, value: any, callback: any) => {
  if (
    (formData.value.groupId === '' || formData.value.groupId === null) &&
    !formData.value.groupName
  )
    callback(new Error(I18N.$fanyi('请选择您的集团')))
  callback()
}
...
const title = computed(() => {
  if (type.value === MODEL_OPERATOR_TYPE.新增) {
    return I18N.$fanyi('新增法人')
  } else {
    return I18N.$fanyi('编辑法人')
  }
})
/**
 * 打开对话框
