...
export default {
  ...
  methods: {
    ...
    getDynamicTemplate() {
      ...
      wordData.YFGZNRLXA = this.pageForm.ZGZNR === '1' ? I18N.$fanyi('是') : '/' //乙方工作内容确定为（填“是”）

      wordData.YFGZNRLXB = this.pageForm.ZGZNR === '2' ? I18N.$fanyi('是') : '/' //乙方工作内容确定为（填“是”）

      wordData.YFGZDD = this.pageForm.ZGZDD //乙方工作地点

      wordData.JYDJZFS = this.pageForm.ZGZZ //（甲、乙双方同意按以下第

      wordData.JYN = this.pageForm.ZGZZJSZQ === '1' ? I18N.$fanyi('年') : '/' //综合计算工时工作制，即经劳动行政部门审批，乙方所在岗位实行以（填“是”）：年

      wordData.JYBN = this.pageForm.ZGZZJSZQ === '2' ? I18N.$fanyi('半年') : '/' //半年

      wordData.JYJ = this.pageForm.ZGZZJSZQ === '3' ? I18N.$fanyi('季') : '/' //季

      wordData.JYY = this.pageForm.ZGZZJSZQ === '4' ? I18N.$fanyi('月') : '/' //月
    ...
    getPdf(state) {
      this.$refs['pageForm'].validate((valid) => {
        if (valid) {
          ...
          wordData.YFGZNRLXA =
            this.pageForm.ZGZNR === '1' ? I18N.$fanyi('是') : '/' //乙方工作内容确定为（填“是”）

          wordData.YFGZNRLXB =
            this.pageForm.ZGZNR === '2' ? I18N.$fanyi('是') : '/' //乙方工作内容确定为（填“是”）
          ...
          wordData.JYN =
            this.pageForm.ZGZZJSZQ === '1' ? I18N.$fanyi('年') : '/' //综合计算工时工作制，即经劳动行政部门审批，乙方所在岗位实行以（填“是”）：年

          wordData.JYBN =
            this.pageForm.ZGZZJSZQ === '2' ? I18N.$fanyi('半年') : '/' //半年

          wordData.JYJ =
            this.pageForm.ZGZZJSZQ === '3' ? I18N.$fanyi('季') : '/' //季

          wordData.JYY =
            this.pageForm.ZGZZJSZQ === '4' ? I18N.$fanyi('月') : '/' //月
