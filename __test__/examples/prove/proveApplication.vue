<template>
  <div>
    <el-row>
      <el-form size="mini" ref="queryForm" label-width="125px" :model="pageData">
        <div class="">
          <div v-if="parentData.proveId === '40'">
            <el-col class="el-col-sm-24">
              <el-form-item
                label="户口迁入地公安局"
                prop="psb"
                :class="{ 'has-input-error': isValidationActive && !pageData.psb }"
              >
                <el-input v-model="pageData.psb" name="psb" style="width: 100%"></el-input>
              </el-form-item>
            </el-col>
            <el-col class="el-col-sm-24">
              <el-form-item
                label="详细地址"
                prop="detailAddress"
                :class="{ 'has-input-error': isValidationActive && !pageData.detailAddress }"
              >
                <el-input v-model="pageData.detailAddress" name="detailAddress" style="width: 100%"></el-input>
              </el-form-item>
            </el-col>
          </div>
          <!--自定义证明的时候展示-->
          <div v-if="parentData.proveId === '99'">
            <el-col class="el-col-md-17 mr10">
              <el-form-item label="文件路径" label-width="100px">
                <el-input type="text" size="mini" disabled style="width: 100%" v-model="filename"></el-input>
              </el-form-item>
            </el-col>
          ...
          <!--在职证明-出国旅游   板块时，21和23需要显示-->
          <div v-if="parentData.proveId === '12' || parentData.proveId === '14'" class="text-center">
            <el-col class="el-col-sm-17">
              <el-form-item label="旅游国家" label-width="100px">
                <el-input type="text" size="mini" style="width: 100%" v-model="pageData.gj"></el-input>
              </el-form-item>
            </el-col>
          ...
          <el-col class="el-col-sm-24 text-center" v-if="parentData.proveId === '99'">
            <el-button type="info" size="mini" :loading="loading" @click="customProveOperation('Y')">
              预览
            </el-button>
            <el-button
              type="primary"
            ...
            <el-button type="danger" size="mini" :loading="loading" @click="closeDialog">
              关闭
            </el-button>
          </el-col>
          <el-col class="el-col-sm-24 text-center" v-else>
            <el-button type="info" size="mini" :loading="loading" @click="submit('Y')">
              预览
            </el-button>
            <el-button
              type="primary"
...
export default {
  ...
  methods: {
    ...
    submit(state) {
      ...
      if (this.parentData.proveId === "40" && (!this.pageData.psb || !this.pageData.detailAddress)) return
      let req = {
        sapInterface: "SH031",
        IV_PERNR:
          this.parentData.MENUII === this.pageCode.ZMKJ ? this.currentUserInfo.PERNR : this.parentData.employeeNum,
      ...
      this.$httpServer.sap.baseMethod(req).then((response) => {
        ...
        if (msgData.MSGTY === "S") {
          ...
          wordData.ZMRQ = this.strDateToChineseDate(new Date().Format("yyyy-MM-dd")) //证明日期
          ...
          if (this.parentData.proveId === "11") {
            ...
            wordData.PLANS = respData.PLANS //职位
            wordData.NSR = respData.ZYX //月薪
            wordData.BEGDA = this.$moment(this.pageData.begda).format("YYYY/MM/DD") //旅游开始日期
            wordData.ENDDA = this.$moment(this.pageData.endda).format("YYYY/MM/DD") //旅游结束日期
            ...
            params.wordName = "收入证明" + "_" + wordData.NAME + "_" + wordData.FILEDATE
            params.wordParams = wordData
            ...
            wordData.BEGDA = this.$isEmpty(respData.BEGDA)
              ? ""
              : new Date(respData.BEGDA.replace(/^(\d{4})(\d{2})(\d{2})$/, "$1-$2-$3")).Format("yyyy/MM/dd")
            wordData.ZNX = respData.ZNX_YEAR //年薪
            wordData.PLANS = respData.PLANS //职位
            wordData.GJ = this.pageData.gj //国家
            wordData.KSRQ = this.$isEmpty(this.pageData.begda) ? "" : new Date(this.pageData.begda).Format("yyyy/MM/dd") //开始日期
            wordData.JSRQ = this.$isEmpty(this.pageData.endda) ? "" : new Date(this.pageData.endda).Format("yyyy/MM/dd") //结束日期
            params.templateName = "cglyzm_sp.ftl"
            params.wordName = "Letter of Guarantee_" + wordData.NAME + "_" + wordData.FILEDATE
            params.wordParams = wordData
          }
    ...
    strDateToChineseDate(strDate) {
      ...
      const chStr = numStr[5] === "0" ? chinese[10] : chinese[10] + chinese[numStr[5]]
    ...
    customProveOperation(state) {
      if (!this.$isEmpty(this.customProveFile) && JSON.stringify(this.customProveFile) !== "{}") {
        if (state === "Y") {
          ...
          if (state === "T") {
            this.$confirm(
              ...
              //提交证明更改状态
              this.submitProve(this.customProveFile.id, this.customProveFile.name, state)
            })
          } else if (state === "D") {
            this.$confirm(
              "是否确定下载打印" +
