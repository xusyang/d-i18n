 ...
 * @LastEditTime: 2021-07-06 17:13:14
 * @Description:  发送信息采集链接 弹出页面
-->
<template>
  <div>
    ...
    <div class="text-center pd20">
      <el-col>
        <el-button size="mini" type="warning" @click="confirmUrging">确认</el-button>
        <el-button size="mini" type="danger" @click="closeDialog">取消</el-button>
      </el-col>
    </div>
    <send-email v-show="false" ref="emailID" :paContractList="parentData" :pageForm="contractList"></send-email>
  </div>
</template>
          ...
          if (es_message.MSGTY === "S") {
            ...
            if (this.contractList.ZZSYKSRQ && this.contractList.ZZHTZZRQ) {
              this.contractList.SYQ = this.MonthsBetw(
                this.contractList.ZZSYKSRQ.replace(/(\d{4})(\d{2})(\d{2})/g, "$1-$2-$3"),
                this.contractList.ZZHTZZRQ.replace(/(\d{4})(\d{2})(\d{2})/g, "$1-$2-$3"),
              )
            }
