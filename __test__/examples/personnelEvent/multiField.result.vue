<template>
  ...
  >
    <el-input
      :type="
        fieldItem.FTYPE === 'P' || fieldItem.FTYPE === 'N' ? 'number' : 'text'
      "
      autocomplete="off"
      :class="
      ...
      @change="
        fieldItem.ZZDIS !== 'X'
          ? dataVerification(group, infoTypeItem, fieldItem)
          : ''
      "
      @blur="
        fieldItem.ZZDIS !== 'X'
  ...
  methods: {
    ...
    searchPe(
      ...
      let param = {
        ...
        IV_DATA:
          fieldItem.ZZZJL === 'X'
            ? JSON.stringify(this.getDataSearch(fieldItem, infoTypeItem))
            : '',
      ...
      this.$httpServer.sap.baseMethod(param, null, true).then((response) => {
        ...
        if (et_org.length > 0) {
          for (let i in et_org) {
            ...
            obj.parent = !et_org[i].SOBID
              ? '#'
              : et_org[i].SOBID + '-' + et_org[i].SCLAS
            obj.label =
              et_org[i].STEXT +
              (et_org[i].HR_CJ ? '(' + et_org[i].HR_CJ + ')' : '')
    ...
    change(event, value, field, infty, subty, ACGRP, ACBLK, fieldItem) {
      ...
      if (fieldItem.ZZSHM === 'T') {
        ...
        const df = !isNaN(new Date(value).getTime())
          ? new Date(value).Format('yyyyMMdd')
          : value
            ...
            if (groupList[j].ACGRP === jsonList[i].ACGRP) {
              ...
              for (let k in sonGroup) {
                if (sonGroup[k].ACBLK === jsonList[i].ACBLK) {
                  ...
                  for (let o in inftyList) {
                    if (
                      inftyList[o].INFTY === jsonList[i].INFTY &&
                      inftyList[o].SUBTY === jsonList[i].SUBTY
                    ) {
                      let fieldList =
                        inftyList[o].ZZTAB === 'X'
                          ? inftyList[o].models[i]
                          : inftyList[o].FIELD
    ...
    getDataSearch(field, typeItem) {
      ...
      for (f of fields) {
        if (f.FTYPE === 'D') {
          // 日期类型转换格式
          df = !isNaN(new Date(f.value).getTime())
            ? new Date(f.value).Format('yyyyMMdd')
            : f.value
