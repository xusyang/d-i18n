  ...
  <div v-loading.fullscreen="loading" :element-loading-text="'加载中.....'">
        ...
        <el-row>
          ...
          <el-col :md="24">
            ...
            <el-form-item class="table-mail-editor">
              <div
                :style="{
                  border: (!pageForm.content || pageForm.content === '<p><br></p>') && isActive ? '1px solid red' : '',
                }"
              >
                <wang-editor :default-value="pageForm.content" @getEditorContent="getEditorContent"></wang-editor>
              </div>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-row>
      <div class="pd20 text-center">
        <el-col>
          <el-button size="mini" type="warning" :loading="loading" @click="submitConfirm('F')">
            发送
          </el-button>
...
export default {
  ...
  methods: {
    ...
    submitConfirm() {
      this.isActive = true
      if (this.$isEmpty(this.pageForm.content) || this.$isEmpty(this.pageForm.title)) {
    ...
    sendMessage() {
      let params = {
        addressee: this.parentData.emailAddress, //"13149920325@163.com",//暂时写死
        copyTo: "",
        bcc: "",
    ...
    updateDataStatusByGuid(operaType) {
      ...
      this.$httpServer.sap.baseMethod(params).then(() => {
        this.$router.push({ path: "/rc/event/", query: { MENUII: this.parentData.pageCode } })
      })
    },
    /**
     * @Copyright  DELOITTE
