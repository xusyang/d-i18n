<template>
  <div class="demo-a">
    ...
    <div
      class="bt-box-p"
      v-if="buttonList.length > 0 && isShowButtn && MENUII !== pageCode.RCKGL && MENUII !== pageCode.XXCJYJ"
    >
      <el-button
        class="pa-but-mgtb"
        size="mini"
    ...
    <div class="ibox-content mt10">
      <el-row>
        <el-form class="mt20" size="mini" ref="queryForm" :model="queryForm" label-width="120px">
          <el-row justify="space-between">
            <el-col
              class="el-col-sm-8"
              v-if="MENUII === pageCode.ZPXQ || MENUII === pageCode.RWGL || MENUII === pageCode.FBGG"
            ></el-col>
            <el-col
              class="el-col-sm-8"
              v-else-if="
            ...
            <el-col class="el-col-sm-8" v-else-if="MENUII !== pageCode.RZZB && MENUII !== pageCode.LZSXBL">
              <el-form-item label="员工姓名：" prop="employeeName">
                <el-input v-model="queryForm.employeeName" style="width: 100%"></el-input>
              </el-form-item>
            </el-col>
            ...
            >
              <el-form-item label="岗位" prop="plansID">
                ...
                <Tree
                  ...
                  :readonly="true"
                  :treetype="'YS'"
                  :text.sync="queryForm.plansName"
                  :parameter="{ czbs: 'YS', id: queryForm.orgID }"
            ...
            <template v-if="MENUII === pageCode.RZZB">
              ...
              <el-col class="el-col-sm-12">
                <el-form-item label="任务标题" prop="employeeName">
                  <el-input v-model="queryForm.titeName" style="width: 100%"></el-input>
                </el-form-item>
              </el-col>
              <el-col class="el-col-sm-12">
            ...
            <el-col class="el-col-sm-8" v-if="MENUII !== pageCode.RZZB && MENUII !== pageCode.LZSXBL">
              ...
              >
                ...
                >
                  <template slot-scope="scope">
                    <template v-if="MENUII === pageCode.RZZB && field.prop === 'ZSUR_DATE' && scope.row.ZRED === 'X'">
                      <div class="redLabel">{{ scope.row[field.prop] }}</div>
                    </template>
                    <template
                      v-else-if="MENUII === pageCode.LZSXBL && field.prop === 'ZSUR_DATE' && scope.row.ZRED === 'X'"
                    >
                      <div class="redLabel">{{ scope.row[field.prop] }}</div>
                    </template>
                    ...
                    <div>
                      ...
                      <template v-if="MENUII === pageCode.RZZB">
                        <el-link
                          ...
                          type="primary"
                          v-show="MENUII === pageCode.RZZB && activeCode === 'F'"
                          :underline="false"
                          size="mini"
                          class="mr5"
                        >
                        ...
                        <el-link
                          @click.native="operationEvent(scope.row, 'ENTRY')"
                          type="primary"
                          v-show="MENUII === pageCode.RZZB && activeCode === 'T'"
                          :underline="false"
                          size="mini"
                          class="mr5"
                        >
                      ...
                      <template v-else-if="MENUII === pageCode.PAHTGLJT && scope.row.ZSTAT === 'F'">
                        <el-link
                      ...
                      <template v-else-if="MENUII === pageCode.LZSXBL">
                        <el-link
                          ...
                          type="primary"
                          v-show="MENUII === pageCode.LZSXBL && activeCode === 'F'"
                          :underline="false"
                          size="mini"
                          class="mr5"
                        >
                        ...
                        <el-link
                          @click.native="operationEvent(scope.row, 'LZSXBL')"
                          type="primary"
                          v-show="MENUII === pageCode.LZSXBL && activeCode === 'T'"
                          :underline="false"
                          size="mini"
                          class="mr5"
                        >
                      ...
                      <template v-else>
                        ...
                        <el-link
                          v-show="activeCode === 'F' && MENUII === pageCode.PALZGL && scope.row.UFSFILEID"
                        ...
                        <el-link
                          v-show="activeCode === 'D' && MENUII === pageCode.PAHTDY"
                        ...
                        <el-link
                          v-show="activeCode === 'B' && MENUII === pageCode.PALGGLSP"
                        ...
                        <el-link
                          v-show="activeCode === 'T' && MENUII === pageCode.ZPXQ"
                        ...
                        <el-link
                          v-show="activeCode === 'T' && scope.row.ZSTAT === '0' && MENUII === pageCode.RWGL"
                        ...
                        <el-link
                          v-show="activeCode === 'T' && scope.row.ZSTAT === '0' && MENUII === pageCode.FBGG"
                          @click.native="operationEvent(scope.row, 'EDIT')"
                          type="warning"
                          :underline="false"
                          size="mini"
                        ...
                        <!--通用的删除按钮 0状态1状态V状态 可以删除-->
                        <el-link
                          v-show="
                            activeCode === 'T' &&
                            (scope.row.ZSTAT === '0' || scope.row.ZSTAT === '1' || scope.row.ZSTAT === 'V') &&
                            MENUII !== pageCode.RCKGL &&
                            MENUII !== pageCode.YRZGL &&
                        ...
                        <el-link
                          v-show="
                            MENUII === pageCode.OFGL &&
                            activeCode === 'D' &&
                            (scope.row.ZSTAT === 'B' || scope.row.ZSTAT === 'C' || scope.row.ZSTAT === 'D')
                          "
                          @click.native="operationEvent(scope.row, 'RESEND_OFFER')"
                          type="primary"
                          :disabled="
                            scope.row.ZSTAT === 'C' ||
                            scope.row.ZSTAT === 'D' ||
                            (scope.row.ZCZRQ &&
                              ...
                              new Date() - new Date(scope.row.ZCZRQ.replace(/(\d{4})(\d{2})(\d{2})/g, '$1\/$2\/$3')) >
                                30 * 1000 * 60 * 60 * 24)
                          "
                          :style="
                            scope.row.ZSTAT === 'C' ||
    ...
    >
      ...
      <batch-employment
        ...
        @closeDialog="closeDialog"
      ></batch-employment>
      <resume-import :parentData="parentData" v-if="showDialog === 'resume'" @closeDialog="closeDialog"></resume-import>
      <send-offer :parentData="parentData" v-if="showDialog === 'offer'" @closeDialog="closeDialog"></send-offer>
      <pa-event-urging
        :parentData="parentData"
...
export default {
  ...
  data() {
    return {
      ...
      activeCode: "", //当前table页签code
      searchContent: "", //表格搜索内容
      tableData: [], //表格数据
      currentPage: 1, //当前页
      pageSize: 10, //每页多少条
      MENUII: "", //pageCode
      buttonList: [], //页面button数组
      isShowButtn: false,
      tableSheetList: [], //待办table页签数组
      tableFieldList: [], //表格字段数组
      parentData: {}, //父页面数据（当前页面，用于弹框）
      showDialog: "", //显示弹框
      dialogVisible: false, //弹框控制参数
      dialogTitle: "", //弹框标题
      defaultEmployeeValue: "", //默认的员工值,
      operationWiidth: 100, //操作列的宽度
      isPLANS: false,
      otherOperationLoading: false, //其他操作loading 比如同步大易数据
  ...
  methods: {
    ...
    getQueryParams() {
      ...
      let params = {
        ...
        IV_BEGDA: this.$isEmpty(this.queryForm.beginDate) ? "" : new Date(this.queryForm.beginDate).Format("yyyyMMdd"),
        IV_ENDDA: this.$isEmpty(this.queryForm.endDate) ? "" : new Date(this.queryForm.endDate).Format("yyyyMMdd"),
        IV_ENAME: this.queryForm.employeeName,
        IT_PERNRS: this.$isEmpty(this.queryForm.employeeNum) ? "" : JSON.stringify([this.queryForm.employeeNum]),
    ...
    queryDataList(isClickQuery) {
      ...
      this.$httpServer.sap.baseMethod(params).then((response) => {
        let et_data = []
        if (this.MENUII === this.pageCode.RZZB || this.MENUII === this.pageCode.LZSXBL) {
    ...
    buttonEvent(param) {
      if (param.ZBATCH === "X") {
        ...
        if (this.pageCode.WDGZ === this.MENUII) {
          ...
          url = "/rc/sendNewNeed"
        } else if (this.pageCode.PALGGLSP === this.MENUII && param.FUNC === "LG3") {
          url = "/sp/workshift"
        } else if (this.pageCode.PALGGLSP === this.MENUII && param.FUNC === "LG6") {
          url = "/sp/batchFixed"
        } else if (this.pageCode.PALGGLSP === this.MENUII && param.FUNC === "LG5") {
          url = "/sp/fixedResult"
        } else {
          url = "/pa/getFileds"
        }
      ...
      this.$httpServer.sap
        ...
        .then((response) => {
          ...
          if (!this.$isEmpty(et_list)) {
            ...
            this.activeCode = this.$isEmpty(this.queryForm.activeCode) ? et_list[0]["LISTT"] : this.queryForm.activeCode
            this.getTableField()
    ...
    sign(row) {
      ...
      this.$httpServer.sap
        ...
        .then((response) => {
          ...
          if (this.ET_DZQZINFO.length === 0) return this.$message.error("暂无可签署合同")
    ...
    operationData(rowobject, operationType) {
      ...
      this.$httpServer.sap.baseMethod(params).then((response) => {
        ...
        this.$commitLog({
          ...
          describe:
            this.currentUserInfo.ENAME + "删除" +
            rowobject.ENAME + localStorage.getItem("currentPageText") +
            "的待办数据",
        })
        if (ES_MESSAGE.MSGTY === "S") {
    ...
    openPage(rowobject, operationType) {
      ...
      if (this.pageCode.WDGZ === this.MENUII) {
      ...
      } else if (operationType === "ASSIGN" && this.pageCode.RWGL === this.MENUII) {
        url = "/rc/taskAssign"
      } else if (
        ...
        (this.pageCode.ZPXQ === this.MENUII || this.pageCode.FBGG === this.MENUII) &&
        operationType === "EDIT"
      ) {
        url = "/rc/sendNewNeed"
      ...
      } else if (this.pageCode.PALGGLSP === this.MENUII && rowobject.FUNC === "LG3" && rowobject.ZZHTQXLX_T) {
        // 批量轮岗申请
        url = "/sp/workshift"
        params.GUID = rowobject.ZZHTQXLX_T
      } else if (this.pageCode.PALGGLSP === this.MENUII && rowobject.FUNC === "LG5" && rowobject.ZZHTQXLX_T) {
        // 定岗结果审批
        url = "/sp/fixedResult"
        params.GUID = rowobject.ZZHTQXLX_T
      } else if (this.pageCode.PALGGLSP === this.MENUII && rowobject.FUNC === "LG6" && rowobject.ZZHTQXLX_T) {
        // 批量定岗申请
        url = "/sp/batchFixed"
        params.GUID = rowobject.ZZHTQXLX_T
      } else if (this.pageCode.SPHTXQ === this.MENUII || this.pageCode.SPHTBG === this.MENUII) {
        params.PERNR = rowobject.PERNR
        url = "/pa/getFileds"
      } else {
        url = "/pa/getFileds"
    ...
    openBatchImportPage(paramObject) {
      if (this.MENUII === this.pageCode.LYGL || this.MENUII === this.pageCode.YRZGL) {
        //简历导入
      ...
      this.dialogTitle = this.MENUII === this.pageCode.LYGL ? "正式员工简历导入" : paramObject.FUNCT
      this.dialogTitle = this.MENUII === this.pageCode.YRZGL ? "正式员工简历导入" : paramObject.FUNCT
     ...
     */
    palgUrging(data) {
      this.$httpServer.sap.baseMethod({
          IV_EMAIL: "X",
          IV_GUID: data.GUID,
          IV_STATS: "G",
          sapInterface: "PE103",
    ...
    handleCellClick(row, column, event, cell) {
      ...
      if (
        ...
        column.property === "TSTAT" &&
        (this.activeCode === "C" || this.activeCode === "D" || this.activeCode === "F" || isShowLink)
      ) {
        //调配
        this.$openWorkflowPage(guid, processCode, "ViewUrl")
      } else if (
    ...
    handleDownload() {
      ...
      let tableData = this.$filterTableContent(this.searchContent, this.tableData)
      //参数1=>table表头 参数2=>接口输出字段 参数3=>接口参数 参数4=>自定义表格数据 参数5=>文件名称
      let params = {
        HEAD_FIELD: null,
        EXCEL_HEAD: JSON.stringify(tableFieldList),
        OUTPUT_FIELD: "ET_DATA",
        CUSTOM_DATA: JSON.stringify(tableData), //前台传入导出数据
    ...
    settingOperationWiidth() {
      ...
      if ((this.MENUII === this.pageCode.PAZZGL || this.MENUII === this.pageCode.PAZZGLJT) && this.activeCode === "T") {
    ...
    resignationDownload(row) {
      ...
      this.$downloadFile({ fileId: row.UFSFILEID, fileName: fileName })
    },
  },
  created() {
    //初始化函数
