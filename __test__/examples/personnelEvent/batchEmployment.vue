...
<template>
  <div>
      ...
      <el-form label-width="115px" size="mini">
        <el-row>
          ...
          <el-col class="el-col-sm-2 m-l-xs">
            <el-button size="mini" type="warning" :loading="loading" @click="selectFile">
              浏览
            </el-button>
            <input type="file" name="fileName" v-show="false" multiple="multiple" @change="uploading" />
          </el-col>
        </el-row>
      </el-form>
    </div>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-link @click="deleteRow(scope.$index)" type="primary" :underline="false" size="mini" class="mr5">
              删除
            </el-link>
          </template>
        </el-table-column>
    ...
    <div class="pd20 text-center">
      <el-col>
        <el-button size="mini" type="primary" v-if="previewList.length > 0" :loading="loading" @click="entryImport()">
          确认导入
        </el-button>
...
export default {
  ...
  data() {
    return {
      ...
      previewList: [], //表格数据
      loading: false,
      currentUserInfo: this.$isEmpty(localStorage.CURRENT_USER_INFO) ? {} : JSON.parse(localStorage.CURRENT_USER_INFO), //当前用户信息
    }
  },
  watch: {
    /**
  ...
  methods: {
    ...
    settingTableField() {
      this.tableHeaderList = [
        { label: "姓名", prop: "ENAME", width: 100 },
        { label: "性别", prop: "GESCH", width: 100 },
        { label: "出生日期", prop: "GBDAT", width: 120 },
        { label: "民族", prop: "RACKY", width: 100 },
        { label: "健康状况", prop: "ZZJKZK", width: 120 },
        { label: "婚姻状况", prop: "FATXT", width: 120 },
        { label: "手机号码", prop: "ZSJ_NUM", width: 120 },
        { label: "微信号", prop: "ZVX_NUM", width: 120 },
        { label: "籍贯", prop: "ZZJGSZSF", width: 100 },
        { label: "户口所在地", prop: "ZZHKSZS", width: 150 },
        { label: "政治面貌", prop: "PCODE", width: 120 },
        { label: "入党日期", prop: "BEGDA", width: 120 },
        { label: "现住地址", prop: "ZADDRS", width: 120 },
        { label: "邮政编码", prop: "PSTLZ", width: 120 },
        {
          label: "身份证号",
          prop: "ICNUM",
          width: 120,
        },
        { label: "邮箱地址", prop: "ZEMAIL", width: 120 },
      ]
    },
    /**
     * @Copyright DELOITTE
