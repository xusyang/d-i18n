<template>
  <div>
    ...
    <!--人员基本信息-->
    <div class="ibox-content mt10" v-if="headerDataList.length > 0 && baseParams.MENUII !== pageCode.SPHTBG">
      <el-row type="flex" align="middle">
        <el-col :span="5">
          <div class="leave-avter">
            <img src="../../../assets/image/user_male_l.png" alt="" v-if="!photo" class="img-lg img-circle" />
    ...
    <div>
      <el-form size="mini" class="pa-form-tabs">
        <div class="ibox-content mt10" style="padding-top: 10px" v-if="groupList.length > 0">
          <el-row style="text-align: right; margin-bottom: -27px; z-index: 1000" v-if="!approveBaseParams">
            <el-button
              :type="butItem.COLOR"
              size="mini"
          ...
          <el-tabs v-model="activeCode" :onload="displayFile()">
            <el-tab-pane
              v-for="(group, groupIndex) in groupList.filter((x) => x.ZZHID !== 'X')"
              :key="groupIndex"
              :label="group.ACGRN"
              :name="group.ACGRP"
            >
              <div class="tipsExplain" v-if="ZPTYPE_SP === '02' && baseParams.MENUII === pageCode.WYLZ">
                <p>离职申请说明：</p>
                <p>
                  1）正式员工需提前30天提交离职申请，试用期员工提前3天，并上传本人签字版的《员工离职申请表》；
                </p>
                <p>2）员工操作不便的，可由人行经理代为发起，发起后由HR线下提醒拟离职员工检查报销、财务欠款等款项的结算情况，上传员工本人签字的员工离职申请表；</p>
                <p>
                  3）请注意检查并处理未报销账款、财务欠款等事宜。
                </p>
              </div>
              <div v-for="(sonGroup, sonGroupIndex) in group.ACBLK" :key="sonGroupIndex">
                <!--信息类型-->
                <div
                  class="main-content-box"
                  v-for="(infoTypeItem, infoTypeItemIndex) in sonGroup.INFTY.filter((x) => x.ZZHID !== 'X')"
                ...
                >
                  ...
                  <div class="infoType-content-box">
                    <el-row>
                      <div>
                        <el-col
                          v-for="(fieldItem, fieldItemIndex) in infoTypeItem.FIELD"
                          v-if="infoTypeItem.ZZTAB !== 'X' && fieldItem.ZZHID !== 'X'"
                          :key="fieldItemIndex"
                          :md="$caleMdCount(infoTypeItem.XXCOL, fieldItem.ZZCOL)"
                          :sm="$caleMdCount(infoTypeItem.XXCOL, fieldItem.ZZCOL)"
                          :class="[fieldItem.ZKJLX !== 'TXT' ? 'field-height' : '']"
                        >
                          <div style="padding-bottom: 2px">
                            <label
                              v-show="fieldItem.ZKJLX !== 'BUT' && fieldItem.ZKJLX !== 'DEL'"
                              style="font-size: 13px"
                            >
                              <span style="color: red" v-show="fieldItem.ZZREQ === 'X'">*</span>
                              <span :style="{ color: fieldItem.COLOR ? fieldItem.COLOR : '#606266' }">
                                {{ fieldItem.FIELN }}
                              </span>
                            </label>
                          </div>
                      ...
                      <template v-if="infoTypeItem.ZZTAB === 'X'">
                        <div v-for="(modelItem, modelItemIndex) in infoTypeItem.models" :key="modelItemIndex">
                          <el-col
                            v-for="(fieldItem, fieldItemIndex) in modelItem.filter((x) => x.ZZHID !== 'X')"
                            :key="fieldItemIndex"
                            :md="$caleMdCount(infoTypeItem.XXCOL, fieldItem.ZZCOL)"
                            :sm="$caleMdCount(infoTypeItem.XXCOL, fieldItem.ZZCOL)"
                            :class="[
                          ...
                          <el-col
                            ...
                            v-if="
                              infoTypeItem.ACTIO !== 'DIS' && infoTypeItem.ACTIO !== 'MOD' && baseParams.EV_DISP !== 'X'
                            "
                          >
                            <el-button
                              type="warning"
                              size="mini"
                              @click.native="addModel(infoTypeItem, modelItemIndex)"
                            >
                            ...
                            <el-button
                              type="danger"
                              size="mini"
                              @click.native="deleteModel(infoTypeItem, modelItemIndex)"
                            >
                              <i class="fa fa-minus"></i>
                            </el-button>
                          </el-col>
                          <hr style="width: 85%" v-show="infoTypeItem.models.length - 1 > modelItemIndex" />
                        </div>
                      </template>
                    </el-row>
                  </div>
            ...
            <el-tab-pane label="附件" name="fileTabSheet" v-if="isShowFile">
              <div v-for="(allFileObject, allFileObjectIndex) in allFileList" :key="allFileObjectIndex">
                <div v-if="allFileObject.length > 0">
                  <h4 class="clearfix m-t-md">
                    <span class="li-bg-blue"></span>
                    <span>{{ allFileObjectIndex === 0 ? "必传资料" : '自定义资料' }}</span>
                  </h4>
                  <hr />
                  <div v-if="allFileObjectIndex === 0 && ZPTYPE_SP === '02' && baseParams.MENUII === pageCode.PAZZGLJT">
                    <p>转正申请所需附件说明：</p>
                    <p>1）请下载员工提交转正自评表及转正报告模板，按填写说明完成后进行上传。</p>
                    <p>
                      2） 入职时确认有经商或兼职情况9级以上干部，在入职时未处理完成的，同时附上经商办企业自查表和干部兼职及薪资情况自查表。
                    </p>
                    <div
                      class="husBox"
                      @click="upHusFile(item.FILE_GUID, item.FILE_NAME, item.FILE_TYPE)"
                  ...
                  <el-row>
                    <div
                      ...
                      v-for="(fileObject, fileObjectIndex) in allFileList[allFileObjectIndex].filter(
                        (x) => x.ZXXCJ !== 'X',
                      )"
                      :key="fileObjectIndex"
                    >
                      <div class="font-bold">
                        <span
                          ...
                          v-show="allFileObjectIndex === 1 && !fileObject.isShowInput && fileObject.ZATTN === ''"
                        >
                          点击添加标题
                        </span>
                        <el-tooltip
                        ...
                        >
                          <p
                            @click="allFileObjectIndex === 1 ? (fileObject.isShowInput = true) : ''"
                            class="pa-file-text"
                          >
                            {{ fileObject.ZATTN }}
                          </p>
                        ...
                        <el-input
                          type="text"
                          size="mini"
                          v-show="allFileObjectIndex === 1 && fileObject.isShowInput && baseParams.EV_DISP !== 'X'"
                          v-model="fileObject.ZATTN"
                          @blur="fileObject.isShowInput = false"
                        ></el-input>
                      </div>

                      <div v-for="(fileItem, fileItemIndex) in fileObject.ZACFILE" :key="fileItemIndex">
                        <el-col :span="16">
                          <div class="file-down-box pa-file-text" @click="$previewFile(fileItem.ATTDL, fileItem.ATTNN)">
                            <el-tooltip
                              effect="dark"
                        ...
                        <el-col :span="8">
                          <div class="file-down-operation-box">
                            <span
                              ...
                              title="下载"
                              @click="$downloadFile(fileItem.ATTDL, fileItem.ATTNN)"
                            ></span>
                            <span
                              class="el-icon-delete"
                              title="删除"
                              v-if="fileItem.ATTNN && baseParams.EV_DISP !== 'X'"
                      ...
                      <div
                        ...
                        v-if="fileObject.ZTYPE === 'U' && baseParams.EV_DISP !== 'X'"
                      >
                        <el-upload
                          :disabled="false"
                          :action="$VUE_BASE + 'dhrDownloadService/utility/file/dfs/upload'"
                          class="avatar-uploader"
                          :on-success="(response, file, fileList) => handleSuccess(response, file, fileObject, fileList)"
                          ...
                          :limit="5"
                        >
                          <img src="../../../assets/image/emp_info/upload-file-new.jpg" alt="" />
                        </el-upload>
                      </div>
                    </div>
                    <!--自定义附件添加按钮-->
                    ...
                    <div
                      ...
                      v-if="allFileObjectIndex === 1 && baseParams.EV_DISP !== 'X'"
                    >
                      <div class="upload-file-img-box">
                        <img src="../../../assets/image/emp_info/upload-file-new.jpg" alt="" @click="addFile" />
                      </div>
                    </div>
                  </el-row>
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      fileList: [],
      fileListDefinition: [],
      allFileList: [[], []], //allFileList的第一个元素代表必填资料，第二个元素代表非必填资料
      customFileIndex: 0,
      photoBase64: "",
      buttonDataList: [],
      verificationList: [],
  ...
  methods: {
    ...
    backing() {
      ...
      let params = { MENUII: pageCode }
      if (this.baseParams.backingQueryParams) params.backingQueryParams = this.baseParams.backingQueryParams
      if (
        pageCode === this.pageCode.PAZZGLJT ||
        pageCode === this.pageCode.YGZZ ||
        (pageCode === this.pageCode.LYGL && this.$isEmpty(this.baseParams.backingQueryParams)) ||
        (pageCode === this.pageCode.YRZGL && this.$isEmpty(this.baseParams.backingQueryParams)) ||
        (pageCode === this.pageCode.PAZZGL && this.$isEmpty(this.baseParams.backingQueryParams)) ||
        (pageCode === this.pageCode.PARZGL && this.$isEmpty(this.baseParams.backingQueryParams))
      ) {
        window.history.back()
      } else if (pageCode === this.pageCode.WYLZ) {
        this.$router.push({ path: "/blocHome" })
      } else if (pageCode === this.pageCode.OFGL || pageCode === this.pageCode.RCKGL) {
        this.$router.push({ path: "/rc/event/", query: params })
      } else if (pageCode === this.pageCode.PAHTGL) {
        this.$router.push({ path: "/ht/event/", query: params })
      } else if (pageCode === this.pageCode.PAZZGLJT) {
        this.$router.push({ path: "/py/personnelInfoList/", query: params })
      } else {
        this.$router.push({ path: "/pa/event/", query: params })
      }
    },
    /**
     * @Copyright  DELOITTE
    ...
    getPersonnelBaseData() {
      ...
      //合同续签
      if (this.baseParams.MENUII === this.pageCode.SPHTXQ) param.sapInterface = "PA057"
     ...
     */
    getAcData() {
      let sapInterface = this.baseParams.MENUII === this.pageCode.YRZGL ? "PE068" : "PE004"
      ...
      this.$httpServer.sap.baseMethod(param).then((response) => {
        ...
        //获取抬头信息
        this.baseParams.employeeNum && this.getPersonnelBaseData(this.baseParams.employeeNum)
          ...
          for (let j in group[i].ACBLK) {
            ...
            for (let k in sonGroup) {
              for (let s in fieldData) {
                if (sonGroup[k].INFTY === fieldData[s].INFTY && sonGroup[k].SUBTY === fieldData[s].SUBTY) {
                  try {
        ...
        for (let i in files) {
          ...
          if (files[i].ZZREQ === "X") {
            ...
            if (this.baseParams.MENUII === this.pageCode.PALGGLSP && this.baseParams.STAT === "G") {
    ...
    assignmentSP(SPHTdata, group) {
      ...
      for (const i in group) {
        ...
        for (const j in group[i].ACBLK) {
          const sonGroup = this.$sortInfoList(group[i].ACBLK[j].INFTY, "SORTD", "asc")
          for (const k in sonGroup) {
            ...
            if (sonGroup[k].ZZTAB === "X") {
              ...
              if (models.length < valueList.length) {
                ...
                models = []
                valueList.forEach(() => models.push(JSON.parse(JSON.stringify(smodels))))
              ...
              for (const o in valueList) {
                ...
                for (const l in models[o]) {
                  ...
                  if (models[o][l].FTYPE === "D") {
                    // 日期类型转换格式
                    models[o][l].value =
                      models[o][l].value && models[o][l].value !== "00000000" ? models[o][l].value : ""
                  } else if (models[o][l].ZZSHM === "D") {
                    //下拉列表转换格式
                    try {
                      if (models[o][l].value) {
                        models[o][l].value = JSON.parse(values[models[o][l].FIELD])
                        ...
                        if (models[o][l].ZZDEF) {
                          ...
                          models[o][l].valueList = []
                          models[o][l].valueList.push(JSON.parse(models[o][l].ZZDEF))
                        } else {
                          models[o][l].value = ""
                          models[o][l].valueList = []
                        }
                    ...
                    try {
                      if (models[o][l].value) {
                        ...
                        models[o][l].value = value.KEY ? value.KEY + "-" + value.VALUE : ""
                      } else {
                        if (models[o][l].ZZDEF) {
                    ...
                    if (models[o][l].value) {
                      if (+i !== 0) {
                        models[o][l].ZZBDS = this.regularReplace(models[o][l].ZZBDS) //处理正则表达式
                      } else {
                        models[o][l].value = values[models[o][l].FIELD]
                          ? values[models[o][l].FIELD].toString().replace(/<br\/>/g, "\n")
                          : ""
                        models[o][l].ZZBDS = this.regularReplace(models[o][l].ZZBDS) //处理正则表达式
                      }
                    } else {
              ...
              for (const o in fields) {
                ...
                if (fields[o].FTYPE === "D") {
                  // 日期类型转换格式
                  fields[o].value = fields[o].value && fields[o].value !== "00000000" ? fields[o].value : ""
                  ...
                  try {
                    if (fields[o].value) {
                      ...
                      fields[o].value = value.KEY ? value.KEY + "-" + value.VALUE : ""
                    }
                  } catch (e) {
                    console.error(
                      new Date().Format("yyyy-MM-dd hh:mm:ss"),
                ...
                } else if (fields[o].ZZSHM === "B" || fields[o].ZZSHM === "S" || fields[o].ZZSHM === "Z") {
                  try {
                    if (fields[o].value) {
                      fields[o].value = JSON.parse(values)
                    } else {
    ...
    processDataGrp(etgrp) {
      const assignValue = (fd) => {
        ...
        if (fd.FTYPE === "D") {
          ...
          try {
            ...
            console.error(new Date().Format("yyyy-MM-dd hh:mm:ss"), "解析", fd.INFTY, fd.FIELD, e)
          }
        ...
        i.ACBLK.forEach((j) => {
          j.INFTY.forEach((k) => {
            ...
            try {
              ...
              console.error(new Date().Format("yyyy-MM-dd hh:mm:ss"), "解析", k.INFTY, k.SUBTY, k.VALUE, e)
              return
            }
            if (k.ZZTAB === "X") {
              //显示的多数据 && k.ACTIO != "DIS"
              ...
              if (valueList.length > 0) {
                for (modelCount = 0; modelCount < k.models.length; modelCount++) {
                  for (fieldCount = 0; fieldCount < k.models[modelCount].length; fieldCount++) {
                    k.models[modelCount][fieldCount].value =
                      valueList[modelCount][k.models[modelCount][fieldCount].FIELD] || ""
                  }
                }
              }
            } else if (valueList.length > 0) {
              for (fieldCount = 0; fieldCount < k.FIELD.length; fieldCount++) {
                k.FIELD[fieldCount].value = valueList[0][k.FIELD[fieldCount].FIELD] || ""
              }
            }
          })
        })
      ...
      group.forEach((gp) => {
        ...
        gp.ACBLK.forEach((j) => {
          ...
          j.INFTY.forEach((k) => {
            if (k.ZZTAB === "X" && k.ACTIO !== "DIS" && k.ACTIO !== "MOD" && k.ACTIO !== "DEL") {
              // 显示的多数据
              k.models.forEach((md) => {
            ...
            } else if (k.ZZTAB === "X" && (k.ACTIO === "DIS" || k.ACTIO === "MOD" || k.ACTIO === "DEL")) {
    ...
    initFileds() {
      ...
      //合同变更、合同续签 只查询模板不查询值
      if (this.baseParams.MENUII === this.pageCode.SPHTBG && this.baseParams.MENUII === this.pageCode.SPHTXQ) {
        params.IV_PERNR = ""
      }
      ...
      for (const i in group) {
        ...
        for (const j in sonGroup) {
          ...
          for (const k in inftyList) {
            const models = inftyList[k].ZZTAB === "X" ? inftyList[k].models : [inftyList[k].FIELD]
            const objectData = {
              ...
              for (const n in fields) {
                if (fields[n].FTYPE === "D") {
                  ...
                  const df = !isNaN(new Date(fields[n].value).getTime())
                    ...
                    : fields[n].value
                  obj[fields[n].FIELD] = this.$isEmpty(fields[n].value) ? "" : df
                } else if (
                  fields[n].ZZSHM === "D" ||
                  fields[n].ZZSHM === "Z" ||
                  fields[n].ZZSHM === "B" ||
                  ...
                  //下拉列表转换格式
                  const val = fields[n].value ? JSON.stringify(fields[n].value) : ""
                  ...
                  if (value) {
                    ...
                    obj[fields[n].FIELD] = JSON.stringify({ KEY: "", VALUE: "" })
                  }
                } else {
                  obj[fields[n].FIELD] = fields[n].value ? fields[n].value.toString().replace(/\n/g, "<br/>") : ""
                }
              }
            ...
            if (inftyList[k].ZZTAB === "X") {
                ...
                for (const n in fields) {
                  if (fields[n].FTYPE === "D") {
                    ...
                    const df = !isNan(new Date(fields[n].value).getTime())
                      ? new Date(fields[n].value).Format("yyyyMMdd")
                      : fields[n].value
                    obj[fields[n].RFIED] = this.$isEmpty(fields[n].value) ? "" : df
                  } else if (
                    fields[n].ZZSHM === "D" ||
                    fields[n].ZZSHM === "Z" ||
                    fields[n].ZZSHM === "B" ||
                    ...
                    obj[fields[n].RFIED] = fields[n].value ? fields[n].value.toString().replace(/\n/g, "<br/>") : ""
                  }
                }
                objectData.VALUE.push(obj)
              }
              ...
              for (const n in fields) {
                if (fields[n].FTYPE === "D") {
                  ...
                  obj[fields[n].RFIED] = fields[n].value ? fields[n].value.replace(/\n/g, "<br/>") : ""
                }
              }
              objectData.VALUE.push(obj)
              objectData.VALUE = JSON.stringify(objectData.VALUE) //装换数据
      ...
      fileList.forEach(response => {
        ...
        if (response.status === "success") {
          ...
          let fileobject = {
            ...
            ATTNN: result.name, //文件名称
            ATTDL: result.id, //文件下载地址
            ATTUP: result.previewUrl, //文件预览地址
            FILEEXT: result.type, //附件类型
    ...
    handleBefore(file, fileObject) {
      ...
      let postf = file.name.substring(file.name.lastIndexOf(".") + 1).toLowerCase()
      ...
      if (fileObject.ZATTT === "140" && postf !== "png" && postf !== "jpg" && postf !== "jpeg") {
        this.$message.warning(
          "寸彩色免冠照片格式应为" + " .png,.jpg,.jpeg",
        )
        return false
      }
    ...
    async doButtonAction(params) {
      ...
      if (params.FCODE === "SEE") {
        ...
        let CURRENT_USER_INFO = JSON.parse(localStorage.getItem("CURRENT_USER_INFO"))
    ...
    async saveData(params) {
      ...
      let saveParams = {
        sapInterface: sapInterface,
        IV_USER: "", //查询员工编号
        IV_PERNR: this.baseParams.employeeNum, //查询员工编号
        IV_ZACTI: this.baseParams.zaction, // 事件编号
        IV_GUID: this.baseParams.GUID,
        IV_STATS: params.FTATS,
        IV_IPAGE: pageCode,
        IV_DATA: JSON.stringify(this.getData()), //页面数据
        IV_URL: "",
      ...
      await this.$httpServer.sap.baseMethod(saveParams).then(async (response) => {
        let es_message = response.ES_MESSAGE
        //保存操作日志
        ...
        this.$commitLog({
          ...
          describe:
            params.FNAME + "了" + localStorage.getItem("currentPageText") + this.baseParams.employeeNum + "的数据",
        ...
        if (es_message.MSGTY === "S") {
          ...
          if (pageCode === this.pageCode.OFGL && params.FCODE === "SEND") {
          ...
          } else if (pageCode === this.pageCode.PAHTGLJT && params.FCODE === "PRIN") {
            this.getTemplateDataDownloadWord("10")
            this.backing()
          } else if (pageCode === this.pageCode.PAHTGLJT && params.FCODE === "DZQZ") {
            this.startSign("10")
          } else {
            ...
            if (params.ACTIO === "K" && !this.$isEmpty(workflowId)) {
              ...
              this.$startWorkFlow({
                ...
                ename: applyName || this.baseParams.ENAME || this.baseParams.employeeNum,
              }).then(() => this.backing())
              ...
              //合同变更、合同续签
              if (this.baseParams.MENUII === this.pageCode.SPHTXQ || this.baseParams.MENUII === this.pageCode.SPHTBG) {
                this.backing()
              } else {
      ...
      if (offerData) {
        ...
        let bdrq =
          offerData.ZZHR_BDRQ && offerData.ZZHR_BDRQ !== "00000000"
            ? new Date(offerData.ZZHR_BDRQ.replace(/^(\d{4})(\d{2})(\d{2})$/, "$1-$2-$3")).Format("yyyy年MM月dd日")
            : ""
        ...
        params.outputDocxFileName = "录用通知函" + "(" + offerData.ENAME + ").doc"
    ...
    initFunction(employeeNum) {
      ...
      if (baseParams) {
        ...
        baseParams.ICNUM = baseParams.ICNUM ? Base64.decode(baseParams.ICNUM) : ""
    ...
    startTignFlow(params) {
      this.$httpServer.file.startTignFlow({ ...params, guId: this.baseParams.GUID }).then((response) => {
    ...
    assemblyWordTemplateParams(data) {
      ...
      params.fileName = "劳动合同模版" + new Date().Format("yyyy-MM-dd") + ".doc"
    ...
    regularReplace(regular) {
      if (regular) {
        return new RegExp((typeof regular == "object" ? regular.source : regular).replace(/\s/g, ""))
      } else {
        return ""
      }
    },
