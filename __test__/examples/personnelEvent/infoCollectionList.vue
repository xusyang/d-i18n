...
Copyright  DELOITTE
Description: 招聘信息采集
   1、当前文件的路径 src/views/pa/personnelEvent/infoCollectionList.vue
...
<template>
  <div>
    ...
    <!-- <div class="bt-box-p">
      <el-button class="pa-but-mgtb" size="mini" type="primary" @click="batchImportData('batch')">
        批量导入入职数据
      </el-button>
      <el-button
        v-if="rPath.indexOf('xxcjksrz') > -1"
        class="pa-but-mgtb"
        size="mini"
        type="info"
        @click="batchImportData('IDcard')"
      >
        快速入职
      </el-button>
    </div> -->
    <div class="ibox-content mt10">
      <el-row>
        <el-form class="mt20" size="mini" :rules="rules" ref="queryForm" :model="queryForm" label-width="120px">
          <el-row justify="space-between">
            <el-col class="el-col-sm-8">
              <el-form-item
                ...
                label="证件类型"
                :show-message="false"
                :class="{ 'has-input-error': isActive && queryForm.credentialsType === '' }"
              >
                <el-select
            ...
            <el-col class="el-col-sm-8">
              <el-form-item
                ...
                label="证件号码"
                :show-message="false"
                :class="{ 'has-input-error': isActive && queryForm.credentialsNum === '' }"
              >
                <el-input v-model="queryForm.credentialsNum" style="width: 100%" />
              </el-form-item>
            </el-col>
            <div class="fr">
              <el-button icon="el-icon-search" type="primary" size="mini" :loading="loading" @click="queryDataList">
                查询
              </el-button>
      ...
      <div class="clearfix mt20">
        ...
        <div style="margin-top: -30px; text-align: right">
          ...
          ></el-input>
          <el-button @click="handleDownload" class="table-buttons-down ml10" size="mini" :loading="exportLoading" round>
            下载表格内容
          </el-button>
        </div>
        <div class="mt10">
              ...
              <template slot-scope="scope">
                <div>
                  <el-button
                    ...
                    type="text"
                    size="mini"
                    v-if="scope.row.ZZHRJZLY !== 'X' && scope.row.ZSFZZ === '否' && scope.row.ZSFCRZ === '否'"
                  >
                    重入职
                  </el-button>
                  <el-button @click.native="checkEmployeeInfo(scope.row)" type="text" size="mini">
                    查看
                  </el-button>
                </div>
              </template>
...
export default {
  ...
  methods: {
    ...
    async queryDataList() {
      ...
      this.isActive = true
      if (!this.queryForm.credentialsType || !this.queryForm.credentialsNum) return
      this.currentPage = 1
      this.tableData = []
      //更新表头
    ...
    initFunction() {
      ...
      if (querySourse && (querySourse.credentialsType || querySourse.credentialsNum)) {
        this.queryForm = {
    ...
    checkEmployeeInfo(rowobject) {
      ...
      let params = {}
      url = "/ess/maintain"
      params = { MENUII: this.MENUII, PERNR: rowobject.PERNR }
    ...
    openDialog(rowObject, flag) {
      ...
      //证件号码 进行base64加密
      this.parentData.credentialsNum = Base64.encode(this.queryForm.credentialsNum)
      if (flag === "X") {
    ...
    getCredentialsTypeList() {
      this.$httpServer.sap.getDicts({ type: "9003", _t: Date.parse(new Date()) / 1000 }).then(res => {
