<template>
  <div class="search-bank-wrap">
    ...
    <el-dialog title="银行搜索" :visible.sync="dialogVisible" width="740px" :append-to-body="true">
      <div class="schbank-header">
        <span class="search-label">银行名称</span>
        <el-input placeholder="银行名称 例如：建设银行*大沙头" v-model="search" @keyup.enter.native="onSearch" />
...
export default {
  ...
  methods: {
    ...
    async onSearch() {
      if (!this.search) {
        this.$alert("请输入银行名称", "提示", { confirmButtonText: "确定" })
        return
      }
      try {
        ...
        this.$notify.error({ title: "错误", message: error.message || "银行信息获取失败！" })
      } finally {
        this.loading = false
      }
    },
    ...
    rowSelect(row) {
      ...
      if (typeof this.value === "string" && this.value !== "") {
        ...
        if (this.inattrs && Object.keys(this.inattrs).length > 0) {
          val = {}
          Object.keys(this.inattrs).forEach(i => val[this.inattrs[i]] = row[i])
        } else {
          val = row
        }
      }
    ...
    tableRowClassName({ row }) {
      let activeCode = typeof this.value === "string" ? this.value : ((this.value || {}).BANKL || "")
